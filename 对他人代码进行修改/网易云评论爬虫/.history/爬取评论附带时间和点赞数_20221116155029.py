...
def hotcomments(html, songname, i, pages, total, filepath):
    ...
    if 'hotComments' in html:
        for item in html['hotComments']:
            ...
            user = item['user']
            # 写入文件
            print("热门评论{}: {} : {}    点赞次数: {} 发布时间：{}".format(m, user['nickname'], item['content'], item['likedCount'],item['timeStr']))
            with open(filepath, 'a', encoding='utf-8') as f:
                f.write("热门评论{}: {} : {}   点赞次数: {} 发布时间：{}\n".format(m, user['nickname'], item['content'], item['likedCount'],item['timeStr']))
                # 回复评论
...
def comments(html, songname, i, pages, total, filepath):
    ...
    for item in html['comments']:
        ...
        if item['timeStr'] =="刚刚":
            ...
            item['timeStr']=datetime.date.today()
        '''
        print("全部评论{}: {} : {}    点赞次数: {} 发布时间：{}".format(j, user['nickname'], item['content'], item['likedCount'],item['timeStr']))
        with open(filepath, 'a', encoding='utf-8') as f:

            f.write("全部评论{}: {} : {}   点赞次数: {} 发布时间：{}\n".format(j, user['nickname'], item['content'], item['likedCount'],item['timeStr']))
            # 回复评论
            if len(item['beReplied']) != 0:
                for reply in item['beReplied']:
                    # 提取发表回复评论的用户名
...
def read_csv():
    ...
    with open("songsid.csv", "r", encoding="utf-8") as csvfile:

        reader = csv.reader(csvfile)
        for row in reader:
            songid, songname = row
            if str(songid) == "歌手名字"|| str(songid)==" ":
                continue
            else:
...
def main():
    for readcsv in read_csv():
        ...
        # 文件存储路径
        filepath = "comments/"+songname + ".txt"
        page = 1
        ...
        while page <= pages:
            if count == 5:
                print('该歌词评论爬取已经失败五次，放弃本次爬取，进行一下首歌曲爬取')
                break
            params, encSecKey = get_params(page)
            data = {'params': params, 'encSecKey': encSecKey}
            html = get_comments_json(url, data)
            # 从第二页开始获取评论
            if False==comments(html, songname, page, pages, total, filepath):
                count=count+1
            page += 1
            time.sleep(1)
