...
#      f.write("正在获取歌曲{}的第{}页评论,总共有{}页{}条评论！\n".format(songname, i, pages, total))
    ...
    if 'hotComments' in html:
        for item in html['hotComments']:
            ...
            user = item['user']
            # 写入文件
            print("热门评论: {}".format(item['content']))
            with open(filepath, 'a', encoding='utf-8') as f:
                f.write("热门评论:{}\n".format(item['content']))
                # 回复评论
...
def comments(html, songname, i, pages, total, filepath):
    ...
    for item in html['comments']:
        ...
        '''
        print("全部评论: {} ".format(item['content']))
        with open(filepath, 'a', encoding='utf-8') as f:
...
def main():
    for readcsv in read_csv():
        ...
        # 文件存储路径
        filepath = "chun-comments/"+songname +"___纯评论"+ ".txt"
        page = 1
